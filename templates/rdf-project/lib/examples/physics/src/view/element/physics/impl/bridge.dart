...
class Bridge extends AbstractBox2dElement implements IBox2dElement {
  ...
  void createBodies() {
    ...
    for (i = 0; i < doublePlanks; ++i) {
      bd.position.Set((100 + plankWidthHalf + plankWidth * i) / m_physScale,
          plankY / m_physScale);
    ...
    anchor.Set(
        (100 + plankWidth * doublePlanks) / m_physScale, plankY / m_physScale);
    jd.Initialize(prevBody, ground, anchor);
    m_world.CreateJoint(jd);

    // Spawn in a bunch of crap
    for (i = 0; i < 5; i++) {
      body = createBox("box_${i}", 60.0, MdDimensions.HEIGHT_BUTTON / 2,
          _randomX(), new Random().nextDouble() * (spanHeight / 2 - 100) + 50,
          density: 1.0,
          friction: 0.3,
          restitution: 0.1,
          angle: 0.0,
    ...
    for (i = 0; i < 5; i++) {
      body = createCircle("ball_${i}", 30.0, _randomX(),
    ...
    for (i = 0; i < 15; i++) {
      ...
      body = createShape(
          "crap_2_${i}", vxs, _randomX(), new Random().nextDouble() * 150 + 50,
