...
abstract class MBox2dHelper {
  ...
  b2Body createBox(
      String name, num halfWidth, num halfHeight, num xCenter, num yCenter,
  ...
  b2Body createShape(
      String name, List<b2Vec2> vertices, num xCenter, num yCenter,
      {int type: b2Body.b2_dynamicBody,
      double density: 1.0,
      double friction: 0.2,
      double restitution: 0.0,
  ...
  void createJoint(b2Body body1, b2Body body2, double lowerAngle,
      double upperAngle, double x, double y) {
      ...
      for (int k = 0; k < b.m_fixtureCount; k++) {
        ...
        switch (s.m_type) {
          case b2Shape.e_circleShape:
            sprite = _createCircleSprite(
                b.GetUserData(), s.m_radius * m_physScale, b);
            break;
          case b2Shape.e_polygonShape:
            ...
            sprite = _createPolySprite(
                b.GetUserData(), vertices, halfWidth, halfHeight, b);
            break;
        }
  ...
  Sprite _createCircleSprite(renderMode, double r, b2Body body) {
    ...
    switch (renderMode) {
      case Box2DRenderMode.PAPER:
        sprite = new MdFab(MdIcon.white(_randomIcon()),
            bgColor: MdColor.RED, radius: r);
        sprite.pivotX = r;
        sprite.pivotY = r;
        break;
  ...
  Sprite _createPolySprite(renderMode, List<b2Vec2> vertices, double halfWidth,
      double halfHeight, b2Body body) {
    Sprite sprite;
    switch (renderMode) {
      case Box2DRenderMode.PAPER:
      ...
      default:
        ...
        sprite.graphics
            .moveTo(vertices[0].x * m_physScale, vertices[0].y * m_physScale);
        for (int i = 1; i < vertices.length; i++) {
          sprite.graphics
              .lineTo(vertices[i].x * m_physScale, vertices[i].y * m_physScale);
        }
        sprite.graphics
