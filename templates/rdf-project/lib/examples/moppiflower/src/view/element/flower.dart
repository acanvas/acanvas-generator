...
class Flower extends SpawnItem {
  ...
  Flower(this.position, this.image, this.startTime, this.degree,
      {this.type: 1, int color: null})
      : super() {
  ...
  void draw(Sprite s, MoppiFlowerModel model, {bool style: false}) {
    ...
    double scale = (interpolate(
                startTime, startTime + 400, elapsedTime.toDouble()) -
            0.5 *
                interpolate(
                    startTime + 400, startTime + 700, elapsedTime.toDouble()) +
            0.2 *
                interpolate(startTime + 700, startTime + 1000,
