...
class Stem extends Base {
  Stem(int stemLength, double stemCurve, Point startPosition,
      ...
      if (rand < 0.10) {
        ...
        if (rand >= 0.06)
          flowerManager.spawnItems.add(new Flower(
              spawnPoint, img, startTimeSpawn, degrees(angle),
              type: type, color: color));
        else
          flowerManager.branches.add(new Branch(
              random.nextInt(20) + 15,
              RandRangeSnap(-9.0, 9.0, 2.0),
              spawnPoint,
              new Point(branchAngle.x, branchAngle.y),
              startTimeSpawn.round(),
              flowerManager,
              color: color));
      }
    }
  ...
  void draw(Sprite s, MoppiFlowerModel model, {bool style: false}) {
    ...
    if (elapsedTime < growDuration + lifeDuration + deathDuration) {
      ...
      _stroke(g, style, color,
          .6 + (color == null ? 6 * model.mid : 0.0)); //strength: 10
