 ...
 * @author nilsdoehring
 */
class UGCRegister extends UGCAbstractLayer implements IFBModelAware, IGoogleModelAware{
  String _imageName;
  MdInput _user;
  MdInput _email;
  MdInput _pass;
  ...
  void init({Map<String, String> params: null}) {
    ...
    _imageName = "ugc-${new DateTime.now().millisecondsSinceEpoch * (1 + new math.Random().nextDouble())}";

    _reflow.addChild(Theme.getHeadline(getProperty("headline"), color: Colors.BLACK));
    _reflow.addChild(Theme.getCopy(getProperty("copy"), color: Colors.BLACK));

    //social login
    _reflow.addChild(Theme.getHeadline(getProperty("social.headline"), size: 20, color: Colors.BLACK));
    _reflow.addChild(Theme.getButton(label: getProperty("button.fb"))
      ..submitEvent = new RdSignal(
          FBEvents.USER_LOGIN,
          new FacebookLoginVO(), _onFB));
    _reflow.addChild(Theme.getButton(label: getProperty("button.gplus"))
      ..submitEvent = new RdSignal(
          GoogleEvents.USER_LOGIN,
          new GoogleLoginVO(
              scopes: [getProperty("plugin.ugc.user.google.scopes.profile", true), getProperty("plugin.ugc.user.google.scopes.email", true)]), _onGoogle));

   //normal signup with u/e/p
    _reflow.addChild(Theme.getHeadline(getProperty("normal.headline"), size: 20, color: Colors.BLACK));

    _user = Theme.getInput(label: getProperty("username"), required : getProperty("username.required"), name: "user");
    _reflow.addChild(_user);

    _reflow.addChild(Theme.getCopy(getProperty("email.desc"), color: Colors.BLACK, size: 14));
    _email = Theme.getInput(label: getProperty("email"), required : getProperty("email.required"), name: "user");
    _reflow.addChild(_email);


    _reflow.addChild(Theme.getCopy(getProperty("password.desc"), color: Colors.BLACK, size: 14));
    _pass = Theme.getInput(label: getProperty("password"), password: true, required : getProperty("password.required"), name: "user");
    _reflow.addChild(_pass);

    _reflow.addChild(Theme.getButton(label: getProperty("button.finish"), shadow: false)
  ...
  void _onGoogle() {
    String middle = _googleModel.user.name.middleName == "" ? "" : _googleModel.user.name.middleName + " ";
    String username = _googleModel.user.name.givenName + " " + middle + _googleModel.user.name.familyName;
  ...
  void _onImageUploaded() {
    new RdSignal(StateEvents.ADDRESS_SET, getProperty("${UGCExampleScreenIDs.UGCHOME}.url", true)).dispatch();
  }
