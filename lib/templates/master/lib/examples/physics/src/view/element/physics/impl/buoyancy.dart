...
class Buoyancy extends AbstractBox2dElement implements IBox2dElement {
  ...
  Buoyancy(b2World world, double worldScale, num spanWidth, num spanHeight)
    ...
    Sprite water = RdGraphics.rectangle(0, 0, spanWidth, spanHeight / 2, color: 0x440000ff);
  ...
  void createBodies() {
    ...
    for (i = 0; i < 5; i++) {
      body = createBox("box_${i}", 60.0, MdDimensions.HEIGHT_BUTTON / 2, new Random().nextDouble() * halfWidth + 0,
    ...
    for (i = 0; i < 5; i++) {
      body = createCircle("ball_${i}", 30.0, new Random().nextDouble() * halfWidth + 0,
    ...
    for (i = 0; i < 15; i++) {
      ...
      body = createShape(
          "crap_${i}", vxs, new Random().nextDouble() * halfWidth + 0, new Random().nextDouble() * 150 + 50,
    ...
    body = createBox("toy_1", 40, 10, 100, halfHeight + 100, density: 3.0, friction: 0.2, restitution: 0.0, angle: 0.0);
    m_bodies.add(body);

    b2BodyDef bodyDef = new b2BodyDef();
    bodyDef.type = b2Body.b2_dynamicBody;
    bodyDef.position.Set((halfWidth + 100) / m_physScale, (halfHeight + 100) / m_physScale);
