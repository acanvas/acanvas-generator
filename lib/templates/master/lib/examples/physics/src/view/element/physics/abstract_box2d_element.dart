...
class AbstractBox2dElement extends BoxSprite with MBox2dHelper {
  ...
  AbstractBox2dElement(b2World world, double worldScale, num spanWidth, num spanHeight) : super() {
    m_world = world;
    m_physScale = worldScale;
    this.spanWidth = spanWidth;
    this.spanHeight = spanHeight;

    Sprite bg = RdGraphics.rectangle(0, 0, spanWidth, spanHeight, color: 0x00000000);
    addChild(bg);

    addEventListener(Rd.TOUCH ? TouchEvent.TOUCH_BEGIN : MouseEvent.MOUSE_DOWN, _onPress);
    addEventListener(Rd.TOUCH ? TouchEvent.TOUCH_MOVE : MouseEvent.MOUSE_MOVE, _onMove);
    addEventListener(Rd.TOUCH ? TouchEvent.TOUCH_END : MouseEvent.MOUSE_UP, _onRelease);
  }
  ...
  void dispose({bool removeSelf: true}) {
    removeEventListener(Rd.TOUCH ? TouchEvent.TOUCH_BEGIN : MouseEvent.MOUSE_DOWN, _onPress);
    removeEventListener(Rd.TOUCH ? TouchEvent.TOUCH_MOVE : MouseEvent.MOUSE_MOVE, _onMove);
    removeEventListener(Rd.TOUCH ? TouchEvent.TOUCH_END : MouseEvent.MOUSE_UP, _onRelease);
    super.dispose();
  }
}
