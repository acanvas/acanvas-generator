...
class MdToasts extends AbstractReflowScreen {
  ...
  void init({Map params: null}) {
    ...
    radioGroupV.addChild(new MdRadioButton(label: getProperty("radio01"), activeColor: Colors.RED));
    radioGroupV.addChild(new MdRadioButton(label: getProperty("radio02")));
    radioGroupV.addChild(new MdRadioButton(label: getProperty("radio03"), activeColor: Theme.COLOR_TRIAD_BLUE));
    radioGroupV.addEventListener(RadioGroupEvent.BUTTON_SELECTED, _onRadioButtonSelected);
    ...
    MdButton button01 = new MdButton(getProperty("button01"),
        fontColor: MdColor.WHITE, bgColor: Colors.PINK, icon: MdIcon.white(MdIconSet.done));
    button01.submitEvent = new RdSignal(StateEvents.MESSAGE_SHOW,
        new StateMessageVO(toast_id, getProperty("toast03"), 0, type: StateMessageVO.TYPE_INFO));
  ...
  void _onRadioButtonSelected(RadioGroupEvent event) {
    switch (event.index) {
      case 0:
        new MdToast(getProperty("toast01"), stage, fontColor: MdColor.WHITE, bgColor: Colors.RED);
        break;
      case 2:
        new MdToast(getProperty("toast02"), stage,
            fontColor: MdColor.WHITE, bgColor: Theme.COLOR_TRIAD_BLUE, position: MdToast.BR);
        break;
    }
  }
}
