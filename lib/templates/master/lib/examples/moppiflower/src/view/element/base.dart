...
class Base extends SpawnItem {
  ...
  Base(this.stemLength, double stemCurve, Point startPosition, Point startDirection, this.startTime,
  ...
  Point getCurrentPosition() {
    ...
    if (numberOfFullSegs < stemLength) {
      ...
      position = new Point(points[numberOfFullSegs].x, points[numberOfFullSegs].y);
    return position;
  }
  ...
  void _stroke(Graphics g, bool style, int color, double size) {
    if (style) {
      g.strokeColor(color ?? rgb2hex((203 * 0.6).round(), (172 * 0.6).round(), (132 * 0.6).round()), 4 * size + 3 + 3,
          JointStyle.ROUND);
    } else {
