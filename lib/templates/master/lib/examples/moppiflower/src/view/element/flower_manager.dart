...
class FlowerManager {
  ...
  FlowerManager(this._res) : super() {
     ...
     */
    Stem stem;
    stem = new Stem(20, 10.0, new Point(400, 400), new Point(5, -10), (Rd.JUGGLER.elapsedTime * 1000.0).round(), this);
    whiteStems.add(stem);

    stem = new Stem(20, 10.0, new Point(350, 450), new Point(5, -10), (Rd.JUGGLER.elapsedTime * 1000.0).round(), this,
        color: Color.Black);
    blackStems.add(stem);
  }
  ...
  void draw(BoxSprite s) {
    ...
    if (whiteStems.length > 0) {
      ...
      model.translation = new Point(s.spanWidth / 2 * 1.1 - smoothedPos.x, s.spanHeight / 2 * 1.1 - smoothedPos.y);
  ...
  void spawnStems() {
    if (whiteStems.length > 0) {
      ...
      if (time >= (startTime + growDuration)) {
        ...
        whiteStems.add(new Stem(
            stemLength, stemCurve, stem.getEndPosition(), stem.getEndDirection(), startTime + growDuration, this));

        Stem stem2 = blackStems.last;
        Point p = new Point(stem.interpolate(stem.getEndDirection().x, stem2.getEndDirection().x, .4),
            stem.interpolate(stem.getEndDirection().y, stem2.getEndDirection().y, .4));
        blackStems.add(new Stem(stemLength, stemCurve * 1.5, stem2.getEndPosition(), p, startTime + growDuration, this,
            color: Color.Black));
      }
    }
  }
