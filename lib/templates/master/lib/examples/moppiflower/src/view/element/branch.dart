...
class Branch extends Base {
  Branch(int stemLength, double stemCurve, Point startPosition, Point startDirection, int startTime,
    ...
    for (int i = 1; i < points.length; i++) {
      ...
      if (rand < 0.05 && (i < (points.length - 3))) {
        ...
        flowerManager.spawnItems
            .add(new Flower(spawnPoint + new Point(-10, -10), img, startTimeSpawn, degrees(angle), type: type));
      }
      if ((i == (points.length - 2))) {
        ...
        angle = math.atan2(branchAngle.y, branchAngle.x);
        flowerManager.spawnItems.add(new Flower(spawnPoint + new Point(-10, -10), img, startTimeSpawn, degrees(angle),
  ...
  void draw(Sprite s, MoppiFlowerModel model, {bool style: false}) {
    ...
    if (elapsedTime < growDuration + lifeDuration + deathDuration) {
      ...
      _stroke(g, style, color, size * .6 + (color == null ? 3 * model.mid : 0.0)); //strength: 10
      ...
      if (numberOfFullSegs < stemLength) {
        ...
        g.lineTo(interpolate(points[numberOfFullSegs].x, points[numberOfFullSegs + 1].x, currentSegRatio),
            interpolate(points[numberOfFullSegs].y, points[numberOfFullSegs + 1].y, currentSegRatio));
